...
class TFile
{
	...
	public:
		TFile ();
		TFile (const TFile &file)  {  init(file);  }
		TFile (const string &filename);

		virtual ~TFile ();

		TFile &operator= (const TFile &file)  {  init(file);  return *this;  }

		const string &Filename () const  {  return filename;  }
		const string &Drive () const  {  return drive;  }
		const string &Path () const  {  return path;  }
		const string &Name () const  {  return name;  }
		const string &Extension () const  {  return extension;  }
		...
		bool IsAbsolute () const  {  return path[0] == DIR_SEP;  }
		bool IsRelative () const  {  return path[0] != DIR_SEP;  }
		bool Exists () const;
