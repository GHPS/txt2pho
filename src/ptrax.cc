       ...
       continue ; }
		...
		if (strchr(ipa_sylcor,in[i]) == NULL) {
			if (strchr(ipa_voicedplos,in[i]) == NULL)
				...
				switch (in[i]) {
					case '6' : result[j++] = 'r'; break;
					case 'd' : result[j++] = 'd'; break;
					case 'g' : result[j++] = 'g'; break;
					case 'b' : result[j++] = 'b'; break;
					case 'z' : result[j++] = 'z'; break;
			...
			if (in[i] == '=') {
				...
				switch (in[++i]) {
					...
					case 'm' : result[j++] = 'm'; break ;
					case 'n' : result[j++] = 'n'; break ;
					case 'N' : result[j++] = 'N'; break ;
					case '6' : result[j++] = 'r'; break ; } }
          else
          	result[j++] = in[i] ; }
