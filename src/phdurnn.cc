...
int get_value(DBSyll* d, int i) {
	switch (i) {
		...
		case 1 : return(d->acat) ; break ;
		case 6 : return(1) ; break ;
		case 3 : return(d->facce) ; break ;
		case 4 : return(d->stress) ; break ;
		case 7 : return(1) ; break ;
		case 9 : return(1) ; break ;
		case 10 : return(1) ; break ;
		case 8 : return(d->pdiss) ; break ;
...
int compute_features(Soundinfo* s, Phonfeatures& f) {
	...
	switch (s->name[0]) {
		...
		case 'S' :
		case 'C' :
		case 'x' : f.kons_art = 1 ; break ;
		case 'm' :
		case 'n' :
		case 'N' :
		case 'l' :
		case 'R' :
		case 'h' : f.kons_art = 2 ; break ;
		case 'b' :
		case 'd' :
		case 'g' :
		case 'p' :
		case 't' :
		case 'k' : f.kons_art = 3 ; break ;
		default : f.kons_art = 0 ; }

	if (strchr("iyu",s->name[0]) != NULL)
		f.of_ge = 4 ;
	else
		if (strchr("e2oIYU",s->name[0]) != NULL)
			f.of_ge = 3 ;
		else
			if (strchr("E9O@=6",s->name[0]) != NULL)
				f.of_ge = 2 ;
			else
				f.of_ge = 1 ;
			...
			if (soundf.vokkns == 1) {							
				...
				for (ix = 0 ; ix < vinputvaluenr ; ix++) {
					...
					switch (vinputvalues[ix].column) {
						case 5: vinputvals[ix] = adjust(b_onc,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 6: vinputvals[ix] = adjust(b_wordclass,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 7: vinputvals[ix] = adjust(nextsoundf.kons_art,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 9: vinputvals[ix] = adjust(nextsoundf.gerundet,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 10: vinputvals[ix] = adjust(nextsoundf.of_ge,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 11: vinputvals[ix] = adjust(lastsoundf.vor_hin,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 15: vinputvals[ix] = adjust(nextsoundf.sh_sl,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 16: vinputvals[ix] = adjust(nextsoundf.vor_hin,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 18: vinputvals[ix] = adjust(soundf.gerundet,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 19: vinputvals[ix] = adjust(soundf.hochtief,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 20: vinputvals[ix] = adjust(soundf.langkurz,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 21: vinputvals[ix] = adjust(soundf.of_ge,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 22: vinputvals[ix] = adjust(soundf.vor_hin,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 23: vinputvals[ix] = adjust(soundf.sh_sl,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 24: vinputvals[ix] = adjust(soundf.kons_art,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 26: vinputvals[ix] = adjust(syll.soundnr-jx,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 27: vinputvals[ix] = adjust(jx+1,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 28: vinputvals[ix] = adjust(lastsoundf.kons_art,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 29: vinputvals[ix] = adjust(lastsoundf.gerundet,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 30: vinputvals[ix] = adjust(lastsoundf.of_ge,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ;
						case 31: vinputvals[ix] = adjust(lastsoundf.sh_sl,vinputvalues[ix].minval,vinputvalues[ix].maxval) ; break ; } 
				...
				if (strcmp(sound->name,"9") == 0) {
				  //fprintf(errfile," im 9-netz ") ;
					oe(vinputvals,outputvals,0) ; }  												
				else
				if (strcmp(sound->name,"NULL") == 0) {
			  ...
			  //fprintf(errfile,"K %2s ",sound->name) ;
				for (ix = 0 ; ix < kinputvaluenr ; ix++) {
					...
					switch (kinputvalues[ix].column) {
						case 5: kinputvals[ix] = adjust(b_onc,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 6: kinputvals[ix] = adjust(b_wordclass,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 7: kinputvals[ix] = adjust(nextsoundf.kons_art,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 9: kinputvals[ix] = adjust(nextsoundf.gerundet,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 10: kinputvals[ix] = adjust(nextsoundf.of_ge,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 11: kinputvals[ix] = adjust(lastsoundf.vor_hin,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 15: kinputvals[ix] = adjust(nextsoundf.sh_sl,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 16: kinputvals[ix] = adjust(nextsoundf.vor_hin,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 18: kinputvals[ix] = adjust(soundf.gerundet,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 19: kinputvals[ix] = adjust(soundf.hochtief,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 20: kinputvals[ix] = adjust(soundf.langkurz,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 21: kinputvals[ix] = adjust(soundf.of_ge,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 22: kinputvals[ix] = adjust(soundf.vor_hin,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 23: kinputvals[ix] = adjust(soundf.sh_sl,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 24: kinputvals[ix] = adjust(soundf.kons_art,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 26: kinputvals[ix] = adjust(syll.soundnr-jx,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 27: kinputvals[ix] = adjust(jx+1,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 28: kinputvals[ix] = adjust(lastsoundf.kons_art,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 29: kinputvals[ix] = adjust(lastsoundf.gerundet,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 30: kinputvals[ix] = adjust(lastsoundf.of_ge,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ;
						case 31: kinputvals[ix] = adjust(lastsoundf.sh_sl,kinputvalues[ix].minval,kinputvalues[ix].maxval) ; break ; }  
